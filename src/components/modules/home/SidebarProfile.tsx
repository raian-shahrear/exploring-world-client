...
const SidebarProfile = ({
  ...
  return (
    <>
      <div className="h-fit">
        {findUser && (
          <div className="h-fit bg-gray-50 rounded-lg p-6 shadow-lg">
            <div className="flex lg:block gap-6 lg:gap-0">
              <div>
                <img
                  ...
                  width={64}
                  height={64}
                  className="w-[150px] h-full lg:h-[150px] lg:mx-auto object-cover object-top border rounded-md"
                />
              </div>
                    ...
                    <p className="text-xs font-semibold mb-2 grid grid-cols-[65px_8px_auto]">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {findUser?.following?.length > 0 && (
                          <button
                            type="button"
                            className="border border-gray-400 h-5 w-5 rounded-md flex justify-center items-center lg:hidden"
                            onClick={() =>
                              setControlFollowing(!controlFollowing)
                            }
                          >
                            {controlFollowing ? (
                              <IoMdArrowDropup />
            ...
            {findUser?.role === "user" && (
              <div
                className={`block lg:hidden ${
                  controlFollowing || controlFollower ? "mt-4" : "mt-0"
                }`}
              >
                <div
                ...
                >
                  ...
                  <div className="mt-2 flex flex-col gap-1 max-h-36 overflow-y-auto overflow-x-hidden">
                    {loggedInUser?.following?.map(
                      (follow: any, idx: number) => (
                        <div
                          key={idx}
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() =>
                              handleUnfollowUser(follow?.user?._id)
                            }
                          >
                            Unfollow
                          </button>
        ...
        <div
          className={`max-h-[450px] overflow-y-auto bg-gray-50 rounded-lg p-6 shadow-lg hidden lg:block ${
            findUser ? "mt-6" : "mt-0"
          }`}
        >
          <PostCategory
